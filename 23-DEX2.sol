...
//we make a new token. 
//and mint 2 tokens. 1 we send it to DEX. 1 is with the user(hack contract)
//Now using the swap formula, we need to find how many tokens we need to send, in order to get 
//100 tokens of token1.
//amount_in*100/1 = 100 => amount_in = 1. Here denominator 1 is the balance of our new tokens in DEX.(You
//can keep it anything, we choose 1)
//since amount_in = 1, we send 1 token while calling swap function. Also, that's why we had to keep 1 with 
//the user(hack contract).